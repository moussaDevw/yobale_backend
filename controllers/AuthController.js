...
exports.sginIn = async (req, res) => {
  try {
    ...
    let { fullName, email, password, phone, image, token } = req.body;
    ...
    if (!hashedPassword) {
      return res
        .status(300)
        .json({ error: true, data: "on peut pa hacher le mot de passe" });
    }
...
exports.signUp = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ error: true, message: "email n'éxiste pas" });
    }
    ...
    if (user.type.name !== "client") {
      return res
        ...
        .json({
          error: true,
          message:
            "Vous n'avez pas l'autorisation d'accéder à la partie client",
        });
    }
    ...
    if (user.deleted) {
      return res
        .status(400)
        .json({ error: true, message: "Ce compte à était suprimer" });
    }
    ...
    if (user.blocked) {
      return res
        ...
        .json({
          error: true,
          message:
            "Ce compte à était blocker par un admin vous ne pouvez pas accédez à votre compte pour le moment",
        });
    }
    ...
    return res
      ...
      .json({ error: false, token, user });
  } catch (error) {
    console.log(error);

    return res
      .status(500)
      .json({ error: true, error, message: "Something went wrong" });
  }
};
  ...
  try {
    ...
    if (user.type.name !== "admin" && user.type.name !== "super admin") {
      return res
        ...
        .json({
          error: true,
          message: "Vous n'avez pas l'autorisation d'accéder à la partie admin",
        });
    }
    ...
    if (user.type.name !== "magasin / restaurant") {
      return res
        ...
        .json({
          error: true,
          message:
            "Vous n'avez pas l'autorisation d'accéder à la partie des réstaurants et magasins",
        });
    }
    ...
    if (user.type.name !== "livreur") {
      return res
        ...
        .json({
          error: true,
          message:
            "Vous n'avez pas l'autorisation d'accéder à la partie des livreurs",
        });
    }
